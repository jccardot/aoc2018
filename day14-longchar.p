...
DEFINE VARIABLE lcRecipes  AS LONGCHAR    NO-UNDO.
...
iElf1 = iRecipeSeq.
iRecipeSeq = iRecipeSeq + 1.
lcRecipes = lcRecipes + "7".
iElf2 = iRecipeSeq.

DO WHILE TRUE:
    iElf1Score = INTEGER(SUBSTRING(lcRecipes, iElf1, 1)).
    iElf2Score = INTEGER(SUBSTRING(lcRecipes, iElf2, 1)).
    iScore = iElf1Score + iElf2Score.
    /* add the recipe(s) to the list */
    lcRecipes = lcRecipes + STRING(iScore).
    IF iScore > 9 THEN
        iRecipeSeq = iRecipeSeq + 2.
    ELSE
        iRecipeSeq = iRecipeSeq + 1.
    /* end condition */
...
cRecipes = STRING(SUBSTRING(lcRecipes, {&xiRecipes} + 1, 10)).
